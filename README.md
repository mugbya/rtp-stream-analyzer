...
- **SIP 信令流程时序图（阶梯图）**：每通通话渲染为信令阶梯图，只保留「谁打给谁」的核心对话（识别结果页和结果报告页均可展开查看）；**通话卡片头部同样直接展示「主叫 → FS → 被叫」链路**，与阶梯图列头身份一致
  ...
  - 按 Call-ID 从各抓包中提取 INVITE/ACK/BYE/CANCEL 等会话内消息，跨抓包去重（利用 CSeq 区分重传与认证重试，过滤 REGISTER/OPTIONS 保活噪声）
  - 每条消息为带箭头横线，箭头指向接收方，线上标注时间与消息类型，按类型着色：请求=蓝、1xx=灰、2xx=绿、3xx~6xx=红
  - **RTP 媒体区间标线（接通后）**：按时间插入信令时间线——绿色开始线落在被叫应答（INVITE 200 OK）之后首个 RTP 包处，灰色结束线落在首条 BYE 之前（末包处），标注起止时间与持续时长。FS 从主叫一呼叫就可能开始收录 RTP（含振铃音/早期媒体），此标线专指**双方接通后真正交换媒体**的区间；无应答信令时回退为整段媒体首末包
